...
{
    S4ADPCM::Error error;
    wav12::ExpanderAD4::compress(codec, samples, nSamples, compressed, nCompressed, S4ADPCM::getTable(codec, table), &error);
    *e12 = error.e16squared;
    ...
    for (int i = 0; i < nSamples; i += STEP) {
        ...
        expander.expand(stereoData + i * 2, n, VOLUME, false, codec, S4ADPCM::getTable(codec, table), true);
    }
    return stereoData;
}
